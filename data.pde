...
int[] cuepointsFrames = new int[cuepoints.length];
...
class Note {
  ...
  Note(int position) {
    pitch = numbers1[position];
    pitchCompressed = numbers2[position];
    tone = notes[position];
    toneCompressed = notes2[position];
    cuePoint = cuepoints[position];
    arpStep = arp[position];
  }
}
